...
const quale=process.argv[2]||'tutto';
fs.mkdirSync(path.join(RAD,'out'),{recursive:true});
/* s0-dati.html e' condiviso: listino, squadra e orari stanno in un posto solo */
if(quale==='sito'||quale==='tutto')
  costruisci(['s1-head.html','s2-body.html','s0-dati.html','s3-script.html'], path.join(RAD,'out','sito.html'));
if(quale==='dash'||quale==='tutto')
  costruisci(['d1-head.html','d2-body.html','s0-dati.html','d3-script.html'], path.join(RAD,'out','dashboard.html'));
